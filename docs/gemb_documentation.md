# gemb documentation
`gemb` runs the Glacier Energy and Mass Balance (GEMB) model. 
...
|-------|-------|-------------|
|**`Profile`** | table |  `Profile` generated by [`model_initialize_profile`](model_initialize_profile_documentation.md).|
...
|**`ClimateForcing`** | timetable | Timetable containing time-series meteorological data|
...
| `ClimateForcing.precipitation_mean`     |  kg m<sup>-2</sup>  |  Climatological mean precipitation per time step   |  
...
| Output | Description
|-------|-------------|
|**`OutData`** | Structure containing output time series.|

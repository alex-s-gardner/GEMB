...
UTC = datestr(mDateVec,'yyyy/mm/dd HH:MM:SS');
  [sAz,sEl] = SolarAzEl(mDateVec,zeros(size(UTC,1),1)+33.8492,zeros(size(UTC,1),1)-118.3875,zeros(size(UTC,1),1));
  plot(sAz,sEl,'.r');
  ...
  ylim([-90 90]);
  subplot(1,2,2);
  plot(mod((0:1440)-420,1440),90-wEl,'.b'); hold on;
  plot(mod((0:1440)-420,1440),90-sEl,'.r');

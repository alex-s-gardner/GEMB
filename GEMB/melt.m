    ...
    if sum(surpT) > 0.0+Ttol % bug fixed 21/07/2016
        ...
        while sum(surpE) > 0.0+Ttol && i<n+1
			   ...
			   if i<n 
					...
					surplusE=surpE(i);
					display([' WARNING: surplus energy at the base of GEMB column' sprintf('\n')])
				end
...
if dm ~= 0 || dE ~= 0
	error(['mass and energy are not conserved in melt equations' sprintf('\n') ' dm: ' ...
		num2str(dm) ' dE: ' num2str(dE) sprintf('\n')])
end

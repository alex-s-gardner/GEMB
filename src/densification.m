...
switch densification_method
    ...
    case 6 % Ligtenberg and others (2011) [semi-emperical], Antarctica
        % common variable
        % From literature: H = exp((-60000.0/(T_mean * R)) + (42400.0/(T_mean * R))) * (C * 9.81);
        H = exp((-60000.0./(T * R)) + (42400.0./(T_mean .* R))) .* (C .* 9.81);
        c0arth = 0.07 * H;
        c1arth = 0.03 * H;
        % ERA5 new albedo_method=1, sw_absorption_method=0
        if albedo_method==1 && sw_absorption_method==0
            if abs(albedo_desnity_threshold - 820.0) < Dtol
        ...
        %From Ligtenberg
        %H = exp((-60000.0/(T_mean * R)) + (42400.0/(T_mean * R))) * (C * 9.81);

"""
    thermo(T, re, dz, d, swf, dlwrf, Ta, V, eAir, pAir, tcIdx, eIdx,
...
- `swf`: Shortwave radiation fluxes [W m⁻²]
- `dlwrf`: Downward longwave radiation fluxes [W m⁻²]
...
"""
function thermo(T, re, dz, d, swf, dlwrf, Ta, V, eAir, pAir, tcIdx, eIdx,
        teValue, dulwrfValue, teThresh, Ws, dt0, dzMin, Vz, Tz, dtScaling, dIce, isdeltaLWup)
...
z0 = if ds < dIce-Dtol && Ws < Wtol
    0.00012        # 0.12 mm for dry snow
elseif ds >= dIce-Dtol
    0.0032         # 3.2 mm for ice
else
    0.0013         # 1.3 mm for wet snow
end
...
if m > 0
    dt = minimum(CI .* dz.^2 .* d ./ (3 .* K) .* dtScaling)
else
    dt = 1e12
end
...
if sum(I) > 0
    ...
    dt = f[1]
    @warn "calculated timestep for thermal loop is < $(f[1]) second. ($(dt) sec)"
end
    ...
    elseif false
        ...
        if Ri > 0.0+Ttol
            ...
            if true #Sjoblom, 2014
                xm = (1.0-19.0*zL)^-0.25
                PhiMz = 2.0*log((1+xm)/2.0) + log((1+xm^2)/2.0) - 2*atan(xm) + π/2
                ...
                xmM = (1.0-16*zLM)^0.25
                xmT = (1.0-16*zLT)^0.25
                PhiMz = 2.0*log((1+xm)/2.0) + log((1+xm^2)/2.0) - 2.0*atan(xm) + π/2.0
                PhiMz0 = 2.0*log((1+xmM)/2.0) + log((1+xmM^2)/2.0) - 2.0*atan(xmM) + π/2.0
...
return shf_cum, lhf_cum, T, EC, ulwrf
